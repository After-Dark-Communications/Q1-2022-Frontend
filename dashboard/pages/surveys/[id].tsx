...
const SurveyPage: React.FC<SurveyByIDProps> = ({ survey }) => {
  ...
  const handleUpdate = async (input: {}) => {
    await axios
      .patch(
        `http://q1-survey-service.herokuapp.com/api/surveys/${survey._id}
    `,
        input
      )
      .then((data) => {
...
export const getStaticPaths = async () => {
  const res = await fetch(
    "http://q1-survey-service.herokuapp.com/api/surveys/"
  );
...
export const getStaticProps = async (context: { params: { id: any } }) => {
  ...
  const response = await fetch(
    "http://q1-survey-service.herokuapp.com/api/surveys/" + id
  );
